...
\author{true}
\date{Versie gecompileerd op 02 Feb 2022}
...
Geraadpleegd op 28 januari 2022 via \url{https://hugoquene.github.io/KMS-NL/} .

Quené, H. \& Van den Bergh, H. (2022). \emph{Kwantitatieve Methoden en Statistiek} {[}in Dutch{]}.
Retrieved 28 January 2022 from \url{https://hugoquene.github.io/KMS-NL/} .
...
\begin{figure}
\includegraphics[width=6.38in]{figures/correlation} \caption{Correlation does not imply causation.}\label{fig:xkcd552}
\end{figure}
...
\includegraphics[width=15.86in]{figures/DBE2012interaction_v2} \textbackslash caption\{Gemiddelde begripsscores (met 95\%-betrouwbaarheidsintervallen), voor twee tekstversies en drie groepen lezers (naar Van Dooren, Van den Bergh en Evers-Vermeul, 2012).\}\label{fig:DBE12interaction}
...
Voor de versie met markeringen zien we dat de cel \texttt{met:zwak} ongeveer gelijk scoort met de cel \texttt{met:gemiddeld} (\(p=.999\)), en dat de cel \texttt{met:sterk} hoger scoort dan de cel \texttt{met:gemiddeld} (\(p=.013\)). Sterke lezers doen het dus beter dan gemiddelde en/of zwakke lezers, bij beide tekstversies.
Ook zien we dat \texttt{zonder:zwak} lager scoort dan \texttt{met:zwak} (\(p<.001\)), dat \texttt{zonder:gemiddeld} lager scoort dan \texttt{met:gemiddeld} (\(p<.001\)), terwijl er geen verschil is tussen \texttt{zonder:sterk} en \texttt{met:sterk} (\(p=.888\)). Sterke lezers hebben dus geen baat bij structuurmarkeringen in de tekst.
...
Voor de versie zonder markeringen zien we dat de cel \texttt{zonder:zwak} lager scoort dan de cel \texttt{zonder:gemiddeld} (\(p<.001\)), en dat de cel \texttt{zonder:sterk} hoger scoort dan de cel \texttt{zonder:gemiddeld} (\(p<.001\)).
Voor de versie met markeringen zien we dat de cel \texttt{met:zwak} ongeveer gelijk scoort met de cel \texttt{met:gemiddeld} (\(p=.999\)), en dat de cel \texttt{met:sterk} hoger scoort dan de cel \texttt{met:gemiddeld} (\(p=.013\)). Sterke lezers doen het dus beter dan gemiddelde en/of zwakke lezers, bij beide tekstversies.
Ook zien we dat \texttt{zonder:zwak} lager scoort dan \texttt{met:zwak} (p\textless.001), dat \texttt{zonder:gemiddeld} lager scoort dan \texttt{met:gemiddeld} (p\textless.001), terwijl er geen verschil is tussen \texttt{zonder:sterk} en \texttt{met:sterk} (p=.888). Sterke lezers hebben dus geen baat bij structuurmarkeringen in de tekst.
Ook interessant is de bevinding dat zwakke lezers een tekst met markeringen net zo goed begrijpen als sterke lezers een tekst zonder markeringen begrijpen (\(p=.722\)).
