...
Uit formule \@ref(eq:t-onesampleCI) volgt logischerwijze, dat als de
...
een populatie-gemiddelde $\overline{y}$ is \begin{equation}
...
gewogen naar hun steekproefomvang. \begin{equation}
    s_p = \sqrt{ \frac{(n_1-1) s^2_1 + (n_2-1) s^2_2} {n_1+n_2-2} }
    (\#eq:sd-pooled)
\end{equation} Vervolgens \begin{equation}
  (\#eq:t-homoskedastic)
...
vrijheidsgraden van $t$ is dan \begin{equation}
  (\#eq:df-WS)
  \nu_\textrm{WS} = 
...
Selecteer de variabelen *cond.u* en *cond.je* in het veld "Variable
...
In formules \@ref(eq:t-onesample) en \@ref(eq:t-pairedsamples) komt tot
...
$(\overline{x_1}-\overline{x_2})$ of $(\overline{D}-\mu_D)$], *en/of*
naarmate de steekproef groter wordt. Kort gezegd [@Rose08 p.338, formule
11.10]: \begin{equation}
  (\#eq:Rose08)
...
> *Voorbeeld 13.2:* In een onderzoek naar de levensduur van overleden
> 50+-ers uit Oostenrijk en Denemarken [@Dobl99] bleek dat de levensduur
...
[^h13toetsing-2]. We vinden zo een gestandaardiseerde effectgrootte van
\begin{equation}
  (\#eq:d-standardized)
    d = \frac{ \overline{x_1}-\overline{x_2} } {s_p} = \frac{10}{90.5} = 0.11
\end{equation} In het eerste voorbeeld hierboven is de
...
dan een willekeurig gekozen jongen, is slechts 0.53 [@McGraw92], en dat
...
[^h13toetsing-2]: In dit geval gebruiken we
    $s_p = \sqrt{ \frac{122\times82^2+128\times98^2} {122+128} } = 90.5$,
    zie formules \@ref(eq:sd-pooled) en \@ref(eq:d-homoskedastic).
...
Als $d$ ligt rond 0.01 spreken we van een zeer klein effect [@Sawi09].
...
0.8 of groter noemen we een groot effect [@Cohen88; @Rose08]. Als $d$
ligt rond 1.2 dan spreken we van een zeer groot effect, en als $d$ ligt
rond 2.0 dan noemen we dat een 'enorm' of 'reusachtig' effect ['huge',
@Sawi09].
...
> *Voorbeeld 13.3:* Kijk nog eens naar formule \@ref(eq:Rose08) en naar
...
gekozen element uit de populatie (formules \@ref(eq:d-onesample) en
\@ref(eq:d-paired)), of (indien van toepassing) voor twee willekeurig en
...
Voor een enkele steekproef (formule \@ref(eq:d-onesample)) kunnen we de
...
gebruik makend van formules \@ref(eq:sd-pooled) en
...
Voor een enkele steekproef (formule \@ref(eq:d-onesample)):\
...
In de uitvoer staat bij `delta` het kleinste significante effect
...
### Betrouwbaarheidsinterval van de effectgrootte {#sec:betrouwbaarheidsinterval-effectgrootte}
...
niet alleen *dat* de voorkeur afwijkt van nul, maar ook *in welke mate*
de (gestandaardiseerde) voorkeur afwijkt van nul, d.w.z. hoe sterk de
voorkeur voor de *je*-versie is. Deze nieuwe kennis over de mate of
